...
public class Pi {
	...
	public static BigDecimal parallelPi(BigDecimal N, BigDecimal threadCount) {
		...
		for(BigDecimal i = BigDecimal.ZERO; i.compareTo(threadCount) < 0; i = i.add(BigDecimal.ONE)) {
			BigDecimal start = (i.multiply(N, MathContext.DECIMAL128)).divide(threadCount,MathContext.DECIMAL128);
			BigDecimal end = ((i.add(BigDecimal.ONE)).multiply(N,MathContext.DECIMAL128)).divide(threadCount,MathContext.DECIMAL128);
